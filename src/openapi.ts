...
export const getMovieDetails = async (movie: ScrappedMovie) => {
  ...
  const response = await client.chat.completions.create({
    model,
    messages: [
      { role: 'system', content: 'You are a helpful assistant that can help me get the details of a movie. You will be given the raw movie data and must return only JSON with the following fields: title, director, year, group, when, summary. The when property must be in the format "YYYY/MM/DD HH:MM" using 24 hours format. Year should be a numeric value. All text should be in Brazilian Portuguese. In some cases, the movie details might have many short movies. In that case, return the event name as the title, director as "Vários", year as the year of the event and extract the different movies names, directors, release dates, durations from the description text and put them in the summary text.' },
